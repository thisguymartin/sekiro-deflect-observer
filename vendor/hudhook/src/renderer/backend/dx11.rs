...
impl D3D11RenderEngine {
    pub fn new(device: &ID3D11Device, ctx: &mut Context) -> Result<Self> {
        ...
        // Record HUD commands in a private context. A partial manual backup of
        // the game's immediate context misses output targets, NULL bindings and
        // resources implicitly unbound by OMSetRenderTargets.
        let device_context = util::try_out_ptr(|out| unsafe {
            device.CreateDeferredContext(0, Some(out))
        })?;
        ...
        // Texture uploads affect only our resources, not the host's bindings.
        let texture_heap = TextureHeap::new(&device, &immediate_context)?;
...
impl RenderEngine for D3D11RenderEngine {
    ...
    fn render(
        ...
        unsafe {
            ...
            let draw_result = self.render_draw_data(draw_data);
            // Release the backbuffer from our deferred context before ResizeBuffers.
            // Finish even on a draw error so partial commands cannot reach a later frame.
            self.device_context.ClearState();
            let commands: Result<ID3D11CommandList> = util::try_out_ptr(|out| {
                self.device_context.FinishCommandList(false, Some(out))
            });
            draw_result?;
            // TRUE asks the D3D runtime to restore the complete host context.
            self.immediate_context.ExecuteCommandList(&commands?, true);
        };
