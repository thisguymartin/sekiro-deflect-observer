...
pub(super) fn draw(
    ...
    let mut position = match target
        .camera
        .project_checked(target.anchor, ui.io().display_size)
    {
    ...
    let viewport_height = display[1].min(display[0] / target.camera.aspect);
    let scale = (viewport_height / 1080.0).min(2.0);
